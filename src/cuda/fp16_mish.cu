...
// 严格遵守所有约束：No FP32 FMA, No tanh, Use FP32 exp, No half RCP
__device__ __forceinline__ __half2 mish_math_pair(__half2 val) {
    ...
    // 同样禁止 FMA
    float2 res;
    float r_x = rsqrtf(__fmul_rn(d.x, d.x));
    float r_y = rsqrtf(__fmul_rn(d.y, d.y));
    res.x = __fmul_rn(x_safe.x, __fmul_rn(n.x, r_x));
    res.y = __fmul_rn(x_safe.y, __fmul_rn(n.y, r_y));
